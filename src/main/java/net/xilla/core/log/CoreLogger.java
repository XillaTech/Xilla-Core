...
public class CoreLogger implements Log {
    ...
    public void log(LogLevel level, String message, Class clazz) {
        if(level.level >= Logger.getLogLevel().level) {
            ...
            if(level == LogLevel.INFO) {
                log = "[" + nowAsISO + " " + Logger.getTimeZone().getDisplayName() + " " + level.name() + "] " + message;
            } else {
                log = "[" + nowAsISO + " " + Logger.getTimeZone().getDisplayName() + " " + level.name() + "] (" + clazz.getName() + ") " + message;
            }
    ...
    public void log(LogLevel level, Throwable message, Class clazz) {
        if(level.level >= Logger.getLogLevel().level) {
            ...
            String log = "[" + nowAsISO + " " + Logger.getTimeZone().getDisplayName() + " " + level.name() + "] (" + clazz.getName() + ") " + message.toString();

            System.out.println(log);

            for(int i = 0; i < message.getStackTrace().length; i++) {
                log = "[" + nowAsISO + " " + Logger.getTimeZone().getDisplayName() + " " + level.name() + "] (" + clazz.getName() + ") [ERR] " + message.getStackTrace()[i].toString();
                System.out.println(log);
            }
        }
    }
